document.addEventListener('DOMContentLoaded', () => {
    ...
    // Array com os nomes e IDs dos Pokémons da primeira geração
    const pokemonData = [
        { id: 1, name: 'Bulbasaur' },
        { id: 2, name: 'Ivysaur' },
        { id: 3, name: 'Venusaur' },
        ...
        { id: 150, name: 'Mewtwo' },
        { id: 151, name: 'Mew' } // Adicionando o Pokémon Mew
    ];
    function showPokemonPage(pageNumber) {
        pokedexList.innerHTML = '';
        const startIndex = pageNumber * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const pokemonPage = pokemonData.slice(startIndex, endIndex);

        pokemonPage.forEach(pokemon => {
            const button = document.createElement('button');
            button.classList.add('pokemon-button');
            button.style.backgroundImage = `url(https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon.id}.png)`;
            button.addEventListener('click', () => showPokemon(pokemon.id));
    ...
    function handleSearch(event) {
        ...
        const filteredPokemon = pokemonData.filter(pokemon => pokemon.name.toLowerCase() === searchTerm);
        if (filteredPokemon.length > 0) {
            showPokemon(filteredPokemon[0].id);
        } else {
    ...
    function updatePaginationButtons() {
        ...
        nextButton.disabled = currentPage === Math.floor(pokemonData.length / itemsPerPage);
    }
    ...
    nextButton.addEventListener('click', () => {
        if (currentPage < Math.floor(pokemonData.length / itemsPerPage)) {
            currentPage++;
            showPokemonPage(currentPage);
            updatePaginationButtons();
        }
